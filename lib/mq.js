...
const AmqpConnectionManager =
  require('amqp-connection-manager').AmqpConnectionManagerClass
...
function sleepUnref(value) {
  ...
  const promise = new Promise(resolve => {
    timer = setTimeout(resolve, value)
    timer.unref()
  })
...
class Mq2 extends EventEmitter {
  ...
  async configure() {
    ...
    const setupTimeouts = (conn, isConsumer) => {
      ...
      conn.on('connect', () => {
        ...
        if (this.findServers) {
          // Clear the cached server urls after being connected for a while.
          // This forces findServers to run on the next reconnect, preventing
          // attempts to connect to decommissioned RabbitMQ servers.
          clearUrlsTimer = setTimeout(() => {
            conn._urls = null
          }, 10 * 60e3).unref()
        }
      })
      ...
      conn.on('disconnect', ({ err }) => {
        ...
        if (typeof err.code !== 'number' || err.code >= 400) {
          ...
          // Error code 320: CONNECTION_FORCED - Server operator initiated
          // connection close. This typically occurs during deployments (e.g.,
          // blue-green) or maintenance. Force use of findServers and reconnect
          // immediately
          if (this.findServers) {
            conn._urls = null
          }

          // TODO: update amqp-connection-manager to use exponential delay for
          // all reconnects
          conn._connect()
        }
      })
  ...
  async handle(options) {
    const { queue, types, handler } = options
    const preHandler = options.preHandler ?? noop
    ...
    await this.consumeChannel.consume(
      ...
      async msg => {
        ...
        const { promise, timer } = sleepUnref(unhandledTimeout)
  ...
  async request(exchange, message, options) {
    ...
    const { promise, timer } = sleepUnref(requestTimeout)
    return Promise.race([
      reply.finally(() => {
        clearTimeout(timer)
      }),
